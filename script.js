...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function requireLogin(){ if(auth.currentUser&&!auth.currentUser.isAnonymous)return true; modal.classList.add('open'); toast('سجّل الدخول أولًا للمتابعة'); return false; }
  ...
  onAuthStateChanged(auth,user=>{const trigger=$('#authTrigger'),text=$('#authTriggerText'),icon=$('.icon',trigger);if(user?.isAnonymous){signOut(auth);return}if(user){const name=user.displayName||user.email?.split('@')[0]||'لاعب';text.textContent=name;icon.textContent=name[0].toUpperCase();trigger.classList.add('signed-in');state.currentPlayer=name;$('#menuName').textContent=name;$('#menuEmail').textContent=user.email||'';$('#menuAvatar').textContent=name[0].toUpperCase()}else{text.textContent='تسجيل الدخول';icon.textContent='♙';trigger.classList.remove('signed-in');$('.account-area').classList.remove('open')}});
