...
import { getAuth, GoogleAuthProvider, signInWithPopup, signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile, sendPasswordResetEmail, onAuthStateChanged, signOut, signInAnonymously } from 'https://www.gstatic.com/firebasejs/12.16.0/firebase-auth.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const state = { selectedTeam: 'red', redName: 'الفريق الأحمر', blueName: 'الفريق الأزرق', maxPlayers: 10, questionCount: 20, questionCategory: 'أسئلة متنوعة', redPlayers: [], bluePlayers: [], currentPlayer: '', roomCode: '', hostUid: '', roomUnsubscribe: null, disconnectTask: null, redScore: 0, blueScore: 0, question: 0, gameQuestions: [], timer: 15, timerId: null, answered: false };
  ...
  function renderLobby(roomData=null) {
    ...
    if(roomData){ state.redName=roomData.redName; state.blueName=roomData.blueName; state.hostUid=roomData.hostUid; state.maxPlayers=Math.min(10,Math.max(2,Number(roomData.maxPlayers)||10)); state.questionCount=Number(roomData.questionCount)||20; state.questionCategory=roomData.questionCategory||'أسئلة متنوعة'; $('#redTeamTitle').textContent=state.redName; $('#blueTeamTitle').textContent=state.blueName; }
  ...
  async function ensurePlayerIdentity(){ if(!auth.currentUser)await signInAnonymously(auth); return auth.currentUser; }
  function watchRoom(code){ if(state.roomUnsubscribe)state.roomUnsubscribe(); const roomReference=ref(db,`rooms/${code}`); state.roomUnsubscribe=onValue(roomReference,snapshot=>{ if(!snapshot.exists()){toast('تم إغلاق الغرفة');state.roomCode='';showView('home');return} const data=snapshot.val();renderLobby(data);if(data.status==='started'&&!$('#gameView').classList.contains('active'))beginGame((data.questionOrder||[]).map(i=>QUESTION_BANK[i]).filter(Boolean)); },()=>toast('تعذر قراءة بيانات الغرفة')); }
  ...
  $('#createForm').addEventListener('submit',async e=>{ e.preventDefault(); const submit=e.submitter;submit.disabled=true;try{await ensurePlayerIdentity();const inputs=$$('.team-input input'),password=$('#createForm input[type="password"]').value;state.redName=inputs[0].value.trim();state.blueName=inputs[1].value.trim();state.maxPlayers=Number($('#playerCount').value);state.questionCount=Number($('.choice-group[data-name="questions"] .selected').textContent);state.questionCategory=$('#category').value;let code;do{code=generateCode()}while((await get(ref(db,`rooms/${code}`))).exists());const leader=$('#captainName').value.trim();await set(ref(db,`rooms/${code}`),{hostUid:auth.currentUser.uid,redName:state.redName,blueName:state.blueName,maxPlayers:state.maxPlayers,questionCount:state.questionCount,questionCategory:state.questionCategory,passwordHash:await hashPassword(password),status:'waiting',createdAt:serverTimestamp(),teams:{red:{[auth.currentUser.uid]:{name:leader,joinedAt:serverTimestamp()}}}});state.selectedTeam='red';await occupySeat(code,'red',leader);toast(`تم إنشاء الغرفة لـ ${state.maxPlayers} لاعبين`)}catch(error){toast(error.code==='auth/operation-not-allowed'?'فعّل تسجيل الدخول المجهول في Firebase':'تعذر إنشاء الغرفة')}finally{submit.disabled=false} });
  $('#joinForm').addEventListener('submit',async e=>{ e.preventDefault(); const submit=e.submitter,name=$('#playerName').value.trim(),code=$('#roomCodeInput').value.trim().toUpperCase();submit.disabled=true;try{await ensurePlayerIdentity();const snapshot=await get(ref(db,`rooms/${code}`));if(!snapshot.exists()){toast('كود الغرفة غير موجود');return}const room=snapshot.val();if(room.status!=='waiting'){toast('المباراة بدأت ولا يمكن الانضمام الآن');return}if(room.passwordHash&&room.passwordHash!==await hashPassword($('#joinPassword').value)){toast('كلمة مرور الغرفة غير صحيحة');return}if(room.teams?.red?.[auth.currentUser.uid]||room.teams?.blue?.[auth.currentUser.uid]){toast('أنت منضم إلى هذه الغرفة بالفعل');return}const maxPlayers=Math.min(10,Math.max(2,Number(room.maxPlayers)||10)),redLimit=Math.ceil(maxPlayers/2),blueLimit=Math.floor(maxPlayers/2),redCount=Object.keys(room.teams?.red||{}).length,blueCount=Object.keys(room.teams?.blue||{}).length;if(redCount>=redLimit&&blueCount>=blueLimit){toast('الغرفة مكتملة');return}const redRatio=redCount/redLimit,blueRatio=blueCount/blueLimit;let assignedTeam;if(redCount>=redLimit)assignedTeam='blue';else if(blueCount>=blueLimit)assignedTeam='red';else if(redRatio===blueRatio)assignedTeam=Math.random()<.5?'red':'blue';else assignedTeam=redRatio<blueRatio?'red':'blue';state.selectedTeam=assignedTeam;await occupySeat(code,assignedTeam,name);toast(`تم توزيعك على ${assignedTeam==='red'?'الفريق الأحمر':'الفريق الأزرق'}`)}catch(error){toast(error.code==='auth/operation-not-allowed'?'فعّل تسجيل الدخول المجهول في Firebase':'تعذر الانضمام الآن')}finally{submit.disabled=false} });
  ...
  async function startGame(){ const redLimit=Math.ceil(state.maxPlayers/2),blueLimit=Math.floor(state.maxPlayers/2);if(state.redPlayers.length!==redLimit||state.bluePlayers.length!==blueLimit){toast(`لا يمكن البدء: الأحمر ${state.redPlayers.length}/${redLimit} والأزرق ${state.bluePlayers.length}/${blueLimit}`);return}if(auth.currentUser?.uid!==state.hostUid){toast('قائد الغرفة فقط يستطيع بدء المباراة');return}let pool=state.questionCategory==='أسئلة متنوعة'?QUESTION_BANK:QUESTION_BANK.filter(q=>q.category===state.questionCategory);pool=[...pool].sort(()=>Math.random()-.5);const selected=pool.slice(0,Math.min(state.questionCount,pool.length)),order=selected.map(q=>q.id);if(selected.length<state.questionCount)toast(`الفئة تحتوي ${selected.length} سؤالًا فقط`);try{await update(ref(db,`rooms/${state.roomCode}`),{status:'started',questionOrder:order,startedAt:serverTimestamp()})}catch{toast('تعذر بدء المباراة')} }
  ...
  function revealAnswer(index){
    ...
    if(correct){ const points=100+state.timer*5; state.redScore+=points; $('#answerStatus').innerHTML=`إجابة صحيحة! +${points} نقطة<br><small>${q.explanation}</small>`; $('#answerStatus').classList.add('correct'); } else { state.blueScore+=80; $('#answerStatus').innerHTML=`${index<0?'انتهى الوقت!':'إجابة غير صحيحة'}<br><small>${q.explanation}</small>`; $('#answerStatus').classList.add('wrong'); }
    $('#redScore').textContent=state.redScore; $('#blueScore').textContent=state.blueScore;
    setTimeout(()=>{ state.question++; if(state.question<state.gameQuestions.length) renderQuestion(); else finishGame(); },3000);
  }
  ...
  onAuthStateChanged(auth,user=>{if(user&&!user.isAnonymous){signOut(auth).then(()=>signInAnonymously(auth))}});
  ensurePlayerIdentity().catch(()=>{});
